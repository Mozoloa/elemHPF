...
HPF1 is a small digital high pass filter audio plugin (VST3/AU) for MacOS and Windows.
...
Next, we fetch the elemHPF project and its dependencies,

```bash
# Clone the project with its submodules
git clone --recurse-submodules https://github.com/Mozoloa/elemHPF
cd elemHPF
...
* After a successful build with either `npm run dev` or `npm run build`, you
  ...
  `./native/build/scripted/HPF1_artefacts` and copy them manually
